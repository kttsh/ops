...
describe('GET /chart-views/:chartViewId/indirect-work-items/:id', () => {
  ...
  test('存在しないIDで 404 を返す', async () => {
    mockedService.findById.mockRejectedValue(
      new HTTPException(404, { message: "Chart view indirect work item with ID '999' not found" }),
    )
  ...
  test('chartViewId 不一致で 404 を返す', async () => {
    mockedService.findById.mockRejectedValue(
      new HTTPException(404, { message: "Chart view indirect work item with ID '1' not found" }),
    )
...
describe('POST /chart-views/:chartViewId/indirect-work-items', () => {
  ...
  test('正常作成で 201 と Location ヘッダを返す', async () => {
    ...
    expect(res.headers.get('Location')).toBe('/chart-views/10/indirect-work-items/1')
  ...
  test('存在しない間接作業ケースIDで 422 を返す', async () => {
    mockedService.create.mockRejectedValue(
      new HTTPException(422, { message: "Indirect work case with ID '999' not found" }),
    )
  ...
  test('重複する間接作業ケースIDで 409 を返す', async () => {
    mockedService.create.mockRejectedValue(
      new HTTPException(409, {
        message: "Indirect work case with ID '100' already exists in chart view '10'",
      }),
    )
