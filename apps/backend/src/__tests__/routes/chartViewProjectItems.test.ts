...
describe("GET /chart-views/:chartViewId/project-items/:id", () => {
	...
	test("存在しないIDで 404 を返す", async () => {
		mockedService.findById.mockRejectedValue(
			new HTTPException(404, {
				message: "Chart view project item with ID '999' not found",
			}),
		);
	...
	test("chartViewId 不一致で 404 を返す", async () => {
		mockedService.findById.mockRejectedValue(
			new HTTPException(404, {
				message: "Chart view project item with ID '1' not found",
			}),
		);
	...
	test("案件ケース所属不一致で 422 を返す", async () => {
		mockedService.create.mockRejectedValue(
			new HTTPException(422, {
				message:
					"Project case with ID '99' not found or does not belong to project '5'",
			}),
		);
...
describe("PUT /chart-views/:chartViewId/project-items/display-order", () => {
	...
	test("所属しないIDが含まれる場合、422 を返す", async () => {
		mockedService.updateDisplayOrder.mockRejectedValue(
			new HTTPException(422, {
				message:
					"Chart view project item with ID '99' does not belong to chart view '10'",
			}),
		);
	...
	test("バリデーションエラー（負の displayOrder）で 422 を返す", async () => {
		const res = await app.request(`${BASE_URL}/display-order`, {
			method: "PUT",
			body: JSON.stringify({
				items: [{ chartViewProjectItemId: 1, displayOrder: -1 }],
			}),
			headers: new Headers({ "Content-Type": "application/json" }),
		});
		expect(res.status).toBe(422);
	...
	test("チャートビュー不存在で 404 を返す", async () => {
		...
		const res = await app.request(
			"/chart-views/999/project-items/display-order",
			{
				method: "PUT",
				body: JSON.stringify({
...
// =============================================================================
// POST - color フィールド対応テスト
// =============================================================================
describe("POST /chart-views/:chartViewId/project-items - color 対応", () => {
	let app: ReturnType<typeof createApp>;
	...
	test("color を指定して作成できる", async () => {
		const itemWithColor = { ...sampleItem1, color: "#ff5733" };
		mockedService.create.mockResolvedValue(itemWithColor);
	...
	test("color 未指定時にレスポンスに color が含まれる", async () => {
		mockedService.create.mockResolvedValue(sampleItem2);
...
// =============================================================================
// PUT /:id - color フィールド対応テスト
// =============================================================================
describe("PUT /chart-views/:chartViewId/project-items/:id - color 対応", () => {
	let app: ReturnType<typeof createApp>;
	...
	test("color を更新できる", async () => {
		const updatedItem = { ...sampleItem1, color: "#10b981" };
		mockedService.update.mockResolvedValue(updatedItem);
	...
	test("color を null に設定できる", async () => {
		const updatedItem = { ...sampleItem1, color: null };
		mockedService.update.mockResolvedValue(updatedItem);
...
// =============================================================================
// GET - color フィールド対応テスト
// =============================================================================
describe("GET /chart-views/:chartViewId/project-items - color 対応", () => {
	let app: ReturnType<typeof createApp>;
	...
	test("レスポンスに color フィールドが含まれる", async () => {
		mockedService.findAll.mockResolvedValue([sampleItem1, sampleItem2]);
		...
		const body = await res.json();
		expect(body.data[0].color).toBe("#3b82f6");
		expect(body.data[1].color).toBeNull();
	});
});

// =============================================================================
// PUT /chart-views/:chartViewId/project-items/bulk - 一括更新
// =============================================================================
describe("PUT /chart-views/:chartViewId/project-items/bulk", () => {
	let app: ReturnType<typeof createApp>;
	...
	test("正常な一括更新で 200 と更新後のアイテム一覧を返す", async () => {
		...
		const res = await app.request(`${BASE_URL}/bulk`, {
			...
			body: JSON.stringify({
				items: [
					...
					{ projectId: 8, displayOrder: 1, isVisible: true },
				],
			}),
			headers: new Headers({ "Content-Type": "application/json" }),
		});
	...
	test("project_case_id が NULL のアイテムを含む一括更新", async () => {
		mockedService.bulkUpsert.mockResolvedValue([sampleItem2]);
	...
	test("存在しない ChartView ID で 404 エラーが返る", async () => {
		mockedService.bulkUpsert.mockRejectedValue(
			new HTTPException(404, { message: "Chart view with ID '999' not found" }),
		);

		const res = await app.request("/chart-views/999/project-items/bulk", {
			method: "PUT",
			body: JSON.stringify({
				items: [{ projectId: 5, displayOrder: 0, isVisible: true }],
			}),
			headers: new Headers({ "Content-Type": "application/json" }),
		});
		expect(res.status).toBe(404);
	...
	test("存在しないプロジェクト ID で 422 エラーが返る", async () => {
		mockedService.bulkUpsert.mockRejectedValue(
			new HTTPException(422, { message: "Project with ID '999' not found" }),
		);
		...
		const res = await app.request(`${BASE_URL}/bulk`, {
			method: "PUT",
			body: JSON.stringify({
				items: [{ projectId: 999, displayOrder: 0, isVisible: true }],
			}),
			headers: new Headers({ "Content-Type": "application/json" }),
		});
		expect(res.status).toBe(422);
	...
	test("バリデーションエラー（projectId が負）で 422 を返す", async () => {
		const res = await app.request(`${BASE_URL}/bulk`, {
			method: "PUT",
			body: JSON.stringify({
				items: [{ projectId: -1, displayOrder: 0, isVisible: true }],
			}),
			headers: new Headers({ "Content-Type": "application/json" }),
		});
		expect(res.status).toBe(422);
	...
	test("サービスの呼び出しパラメータを検証する", async () => {
		...
		await app.request(`${BASE_URL}/bulk`, {
			method: "PUT",
			body: JSON.stringify({
				items: [
					{
