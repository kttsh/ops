...
describe('GET /standard-effort-masters', () => {
  ...
  test('page[number] と page[size] でページネーションする', async () => {
    ...
    const res = await app.request('/standard-effort-masters?page[number]=2&page[size]=1')
    expect(res.status).toBe(200)
  ...
  test('filter[includeDisabled]=true で論理削除済みを含む', async () => {
    ...
    const res = await app.request('/standard-effort-masters?filter[includeDisabled]=true')
    expect(res.status).toBe(200)
  ...
  test('filter[businessUnitCode] でフィルタリングする', async () => {
    ...
    const res = await app.request('/standard-effort-masters?filter[businessUnitCode]=BU-001')
    expect(res.status).toBe(200)
  ...
  test('filter[projectTypeCode] でフィルタリングする', async () => {
    ...
    const res = await app.request('/standard-effort-masters?filter[projectTypeCode]=PT-001')
    expect(res.status).toBe(200)
...
describe('GET /standard-effort-masters/:id', () => {
  ...
  test('存在しない ID で 404 を返す', async () => {
    mockedService.findById.mockRejectedValue(
      new HTTPException(404, { message: "Standard effort master with ID '999' not found" }),
    )
  ...
  test('複合ユニークキー重複で 409 を返す', async () => {
    mockedService.create.mockRejectedValue(
      new HTTPException(409, {
        message: "Standard effort master with name 'Sカーブパターン' already exists for business unit 'BU-001' and project type 'PT-001'",
      }),
    )
...
describe('PUT /standard-effort-masters/:id', () => {
  ...
  test('名称重複で 409 を返す', async () => {
    mockedService.update.mockRejectedValue(
      new HTTPException(409, {
        message: "Standard effort master with name '重複名' already exists for business unit 'BU-001' and project type 'PT-001'",
      }),
    )
...
describe('DELETE /standard-effort-masters/:id', () => {
  ...
  test('参照中で 409 を返す', async () => {
    mockedService.delete.mockRejectedValue(
      new HTTPException(409, {
        message: "Standard effort master with ID '1' is referenced by other resources and cannot be deleted",
      }),
    )
...
describe('POST /standard-effort-masters/:id/actions/restore', () => {
  ...
  test('正常復元で 200 を返す', async () => {
    ...
    const res = await app.request('/standard-effort-masters/1/actions/restore', {
      method: 'POST',
    })
    expect(res.status).toBe(200)
  ...
  test('存在しない ID で 404 を返す', async () => {
    ...
    const res = await app.request('/standard-effort-masters/999/actions/restore', {
      method: 'POST',
    })
    expect(res.status).toBe(404)
