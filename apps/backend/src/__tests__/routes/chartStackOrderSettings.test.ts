...
describe('GET /chart-stack-order-settings/:id', () => {
  ...
  test('存在しないIDで 404 を返す', async () => {
    mockedService.findById.mockRejectedValue(
      new HTTPException(404, { message: "Chart stack order setting with ID '999' not found" }),
    )
  ...
  test('重複する targetType + targetId で 409 を返す', async () => {
    mockedService.create.mockRejectedValue(
      new HTTPException(409, {
        message: "Chart stack order setting with target type 'project' and target ID '1' already exists",
      }),
    )
...
describe('PUT /chart-stack-order-settings/:id', () => {
  ...
  test('更新後の targetType + targetId 重複で 409 を返す', async () => {
    mockedService.update.mockRejectedValue(
      new HTTPException(409, {
        message: "Chart stack order setting with target type 'project' and target ID '2' already exists",
      }),
    )
