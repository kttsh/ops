...
describe("GET /indirect-work-cases", () => {
	...
	test("page[number] と page[size] でページネーションする", async () => {
		...
		const res = await app.request(
			"/indirect-work-cases?page[number]=2&page[size]=1",
		);
		expect(res.status).toBe(200);
	...
	test("filter[includeDisabled]=true で論理削除済みを含む", async () => {
		...
		const res = await app.request(
			"/indirect-work-cases?filter[includeDisabled]=true",
		);
		expect(res.status).toBe(200);
	...
	test("businessUnitCode が不存在で 422 を返す", async () => {
		mockedService.create.mockRejectedValue(
			new HTTPException(422, {
				message: "Business unit with code 'INVALID' not found",
			}),
		);
	...
	test("参照中で 409 を返す", async () => {
		mockedService.delete.mockRejectedValue(
			new HTTPException(409, {
				message:
					"Indirect work case with ID '1' is referenced by other resources and cannot be deleted",
			}),
		);
...
describe("POST /indirect-work-cases/:id/actions/restore", () => {
	...
	test("未削除で 404 を返す", async () => {
		mockedService.restore.mockRejectedValue(
			new HTTPException(404, {
				message: "Indirect work case with ID '1' not found",
			}),
		);
