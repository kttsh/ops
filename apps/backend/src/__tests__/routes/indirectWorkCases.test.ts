...
// --- モック ---
vi.mock('@/services/indirectWorkCaseService', () => ({
  indirectWorkCaseService: {
    findAll: vi.fn(),
    findById: vi.fn(),
    create: vi.fn(),
    update: vi.fn(),
...
const mockedService = vi.mocked(indirectWorkCaseService)
...
const sampleCase2 = {
  ...
  businessUnitName: 'エネルギー事業',
  createdAt: '2026-01-01T00:00:00.000Z',
  updatedAt: '2026-01-01T00:00:00.000Z',
}

describe('GET /indirect-work-cases', () => {
  ...
  test('一覧を data 配列と meta.pagination で返す', async () => {
    ...
    const res = await app.request('/indirect-work-cases')
  ...
  test('page[number] と page[size] でページネーションする', async () => {
    ...
    const res = await app.request('/indirect-work-cases?page[number]=2&page[size]=1')
    expect(res.status).toBe(200)
  ...
  test('filter[includeDisabled]=true で論理削除済みを含む', async () => {
    ...
    const res = await app.request('/indirect-work-cases?filter[includeDisabled]=true')
    expect(res.status).toBe(200)
  ...
  test('不正なクエリパラメータで 422 を返す', async () => {
    const res = await app.request('/indirect-work-cases?page[number]=-1')
...
describe('GET /indirect-work-cases/:id', () => {
  let app: ReturnType<typeof createApp>
  ...
  test('指定IDの間接作業ケースを data オブジェクトで返す', async () => {
    mockedService.findById.mockResolvedValue(sampleCase)

    const res = await app.request('/indirect-work-cases/1')
    expect(res.status).toBe(200)
  ...
  test('存在しないIDで 404 を返す', async () => {
    ...
    const res = await app.request('/indirect-work-cases/999')
...
describe('POST /indirect-work-cases', () => {
  let app: ReturnType<typeof createApp>
  ...
  test('正常作成で 201 と Location ヘッダを返す', async () => {
    ...
    expect(res.headers.get('Location')).toBe('/indirect-work-cases/1')
  ...
  test('バリデーションエラーで 422 を返す（businessUnitCode 未指定）', async () => {
    const res = await app.request('/indirect-work-cases', {
      method: 'POST',
      body: JSON.stringify({
        caseName: 'テスト',
...
describe('PUT /indirect-work-cases/:id', () => {
  let app: ReturnType<typeof createApp>
...
describe('DELETE /indirect-work-cases/:id', () => {
  let app: ReturnType<typeof createApp>
  ...
  test('参照中で 409 を返す', async () => {
    mockedService.delete.mockRejectedValue(
      new HTTPException(409, {
        message: "Indirect work case with ID '1' is referenced by other resources and cannot be deleted",
      }),
    )
...
describe('POST /indirect-work-cases/:id/actions/restore', () => {
  let app: ReturnType<typeof createApp>
  ...
  test('正常復元で 200 を返す', async () => {
    ...
    const res = await app.request('/indirect-work-cases/1/actions/restore', {
      method: 'POST',
    })
    expect(res.status).toBe(200)
  ...
  test('存在しないIDで 404 を返す', async () => {
    ...
    const res = await app.request('/indirect-work-cases/999/actions/restore', {
      method: 'POST',
    })
    expect(res.status).toBe(404)
  ...
  test('未削除で 404 を返す', async () => {
    mockedService.restore.mockRejectedValue(
      new HTTPException(404, { message: "Indirect work case with ID '1' not found" }),
    )
