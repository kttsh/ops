...
describe('businessUnitService', () => {
  ...
  describe('findByCode', () => {
    ...
    test('存在しない場合は HTTPException(404) を投げる', async () => {
      ...
      await expect(businessUnitService.findByCode('NON-EXISTENT')).rejects.toThrow(
        HTTPException,
      )
  ...
  describe('create', () => {
    test('新規ビジネスユニットを作成して camelCase で返す', async () => {
      mockBusinessUnitData.findByCodeIncludingDeleted.mockResolvedValue(undefined)
    ...
    test('同一コードが既に存在する場合は HTTPException(409) を投げる', async () => {
      mockBusinessUnitData.findByCodeIncludingDeleted.mockResolvedValue(sampleRow)
    ...
    test('論理削除済みコードが存在する場合も HTTPException(409) を投げる', async () => {
      mockBusinessUnitData.findByCodeIncludingDeleted.mockResolvedValue(deletedRow)
  ...
  describe('delete', () => {
    test('ビジネスユニットを論理削除する', async () => {
      ...
      await expect(businessUnitService.delete('BU-001')).resolves.toBeUndefined()
    })
