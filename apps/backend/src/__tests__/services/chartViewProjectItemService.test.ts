...
describe("chartViewProjectItemService", () => {
	...
	describe("findAll", () => {
		...
		test("チャートビューが存在しない場合、404 を投げる", async () => {
			...
			await expect(chartViewProjectItemService.findAll(999)).rejects.toThrow(
				HTTPException,
			);
			try {
	...
	describe("findById", () => {
		...
		test("レコードが存在しない場合、404 を投げる", async () => {
			...
			await expect(
				chartViewProjectItemService.findById(10, 999),
		...
		test("chartViewId が不一致の場合、404 を投げる", async () => {
			...
			await expect(chartViewProjectItemService.findById(20, 1)).rejects.toThrow(
				HTTPException,
			);
			try {
	...
	describe("update", () => {
		test("正常に更新して変換済みオブジェクトを返す", async () => {
			...
			const result = await chartViewProjectItemService.update(10, 1, {
				displayOrder: 5,
			});
	...
	describe("delete", () => {
		test("正常に削除する", async () => {
			...
			await expect(
				chartViewProjectItemService.delete(10, 1),
		...
		test("レコードが存在しない場合、404 を投げる", async () => {
			...
			await expect(chartViewProjectItemService.delete(10, 999)).rejects.toThrow(
				HTTPException,
			);
			try {
		...
		test("chartViewId が不一致の場合、404 を投げる", async () => {
			...
			await expect(chartViewProjectItemService.delete(20, 1)).rejects.toThrow(
				HTTPException,
			);
			try {
	...
	describe("bulkUpsert", () => {
		test("正常に一括更新して変換済み配列を返す", async () => {
			...
			const result = await chartViewProjectItemService.bulkUpsert(10, {
				items: [
					{
						projectId: 5,
						projectCaseId: 12,
		...
		test("チャートビュー不存在で 404 を投げる", async () => {
			...
			await expect(
				chartViewProjectItemService.bulkUpsert(999, {
					items: [{ projectId: 5, displayOrder: 0, isVisible: true }],
				}),
			).rejects.toThrow(HTTPException);

			try {
				await chartViewProjectItemService.bulkUpsert(999, {
					items: [{ projectId: 5, displayOrder: 0, isVisible: true }],
				});
			} catch (e) {
				expect((e as HTTPException).status).toBe(404);
		...
		test("存在しない案件IDで 422 を投げる", async () => {
			mockData.chartViewExists.mockResolvedValue(true);
			mockData.projectExists.mockResolvedValue(false);

			await expect(
				chartViewProjectItemService.bulkUpsert(10, {
					items: [{ projectId: 999, displayOrder: 0, isVisible: true }],
				}),
			).rejects.toThrow(HTTPException);

			try {
				await chartViewProjectItemService.bulkUpsert(10, {
					items: [{ projectId: 999, displayOrder: 0, isVisible: true }],
				});
			} catch (e) {
				expect((e as HTTPException).status).toBe(422);
		...
		test("projectCaseId が null の場合、所属検証を実行しない", async () => {
			mockData.chartViewExists.mockResolvedValue(true);
			mockData.projectExists.mockResolvedValue(true);
			mockData.bulkUpsert.mockResolvedValue([sampleRow2]);
