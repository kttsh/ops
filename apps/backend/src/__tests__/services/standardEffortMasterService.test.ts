...
describe('standardEffortMasterService', () => {
  ...
  describe('create', () => {
    test('新規マスタを作成して詳細レスポンスを返す', async () => {
      mockBusinessUnitData.findByCode.mockResolvedValue({ business_unit_code: 'BU-001' })
      mockProjectTypeData.findByCode.mockResolvedValue({ project_type_code: 'PT-001' })
  ...
  describe('update', () => {
    test('名称を更新して詳細レスポンスを返す', async () => {
      ...
      const result = await standardEffortMasterService.update(1, { name: '更新名' })
    ...
    test('名称変更時のユニークチェックで自身は除外する', async () => {
      ...
      const result = await standardEffortMasterService.update(1, { name: 'Sカーブパターン' })
  ...
  describe('delete', () => {
    test('マスタを論理削除する', async () => {
      ...
      await expect(standardEffortMasterService.delete(1)).resolves.toBeUndefined()
    })
