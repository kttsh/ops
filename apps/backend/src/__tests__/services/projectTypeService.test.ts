...
describe('projectTypeService', () => {
  ...
  describe('findByCode', () => {
    ...
    test('存在しない場合は HTTPException(404) を投げる', async () => {
      ...
      await expect(projectTypeService.findByCode('NON-EXISTENT')).rejects.toThrow(
        HTTPException,
      )
  ...
  describe('create', () => {
    test('新規案件タイプを作成して camelCase で返す', async () => {
      mockProjectTypeData.findByCodeIncludingDeleted.mockResolvedValue(undefined)
    ...
    test('同一コードが既に存在する場合は HTTPException(409) を投げる', async () => {
      mockProjectTypeData.findByCodeIncludingDeleted.mockResolvedValue(sampleRow)
    ...
    test('論理削除済みコードが存在する場合も HTTPException(409) を投げる', async () => {
      mockProjectTypeData.findByCodeIncludingDeleted.mockResolvedValue(deletedRow)
  ...
  describe('delete', () => {
    test('案件タイプを論理削除する', async () => {
      ...
      await expect(projectTypeService.delete('PT-001')).resolves.toBeUndefined()
    })
