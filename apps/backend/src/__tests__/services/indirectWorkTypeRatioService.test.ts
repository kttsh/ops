...
describe('indirectWorkTypeRatioService', () => {
  ...
  describe('findAll', () => {
    ...
    test('親ケースが存在しない場合、404 を投げる', async () => {
      ...
      await expect(indirectWorkTypeRatioService.findAll(999)).rejects.toThrow(HTTPException)
      try {
  ...
  describe('findById', () => {
    ...
    test('レコードが存在しない場合、404 を投げる', async () => {
      ...
      await expect(indirectWorkTypeRatioService.findById(10, 999)).rejects.toThrow(HTTPException)
      try {
    ...
    test('親ケースIDが不一致の場合、404 を投げる', async () => {
      ...
      await expect(indirectWorkTypeRatioService.findById(20, 1)).rejects.toThrow(HTTPException)
      try {
  ...
  describe('update', () => {
    test('正常に更新して変換済みオブジェクトを返す', async () => {
      ...
      const result = await indirectWorkTypeRatioService.update(10, 1, { ratio: 0.8 })
    ...
    test('workTypeCode 変更時に複合キー重複で 409 を投げる', async () => {
      ...
      try {
        await indirectWorkTypeRatioService.update(10, 1, { workTypeCode: 'REVIEW' })
      } catch (e) {
        expect((e as HTTPException).status).toBe(409)
      }
    })
    ...
    test('ratio のみの変更時は重複チェックを実行しない', async () => {
      ...
      const result = await indirectWorkTypeRatioService.update(10, 1, { ratio: 0.9 })
  ...
  describe('delete', () => {
    test('正常に削除する', async () => {
      ...
      await expect(indirectWorkTypeRatioService.delete(10, 1)).resolves.toBeUndefined()
    ...
    test('レコードが存在しない場合、404 を投げる', async () => {
      ...
      await expect(indirectWorkTypeRatioService.delete(10, 999)).rejects.toThrow(HTTPException)
      try {
    ...
    test('親ケースIDが不一致の場合、404 を投げる', async () => {
      ...
      await expect(indirectWorkTypeRatioService.delete(20, 1)).rejects.toThrow(HTTPException)
      try {
  ...
  describe('bulkUpsert', () => {
    ...
    test('異なる年度の同一 workTypeCode は重複とみなさない', async () => {
      mockData.indirectWorkCaseExists.mockResolvedValue(true)
      mockData.bulkUpsert.mockResolvedValue([sampleRow, { ...sampleRow2, fiscal_year: 2027 }])
