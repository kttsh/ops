...
describe('headcountPlanCaseService', () => {
  ...
  describe('update', () => {
    ...
    test('存在しない businessUnitCode で HTTPException(422) を投げる', async () => {
      ...
      try {
        await headcountPlanCaseService.update(1, { businessUnitCode: 'INVALID' })
      } catch (e) {
        expect(e).toBeInstanceOf(HTTPException)
        expect((e as HTTPException).status).toBe(422)
      }
  ...
  describe('restore', () => {
    test('論理削除済み人員計画ケースを復元して camelCase で返す', async () => {
      mockHeadcountPlanCaseData.findByIdIncludingDeleted.mockResolvedValue(deletedRow)
    ...
    test('存在しない場合は HTTPException(404) を投げる', async () => {
      mockHeadcountPlanCaseData.findByIdIncludingDeleted.mockResolvedValue(undefined)
    ...
    test('論理削除されていない場合は HTTPException(409) を投げる', async () => {
      mockHeadcountPlanCaseData.findByIdIncludingDeleted.mockResolvedValue(sampleRow)
