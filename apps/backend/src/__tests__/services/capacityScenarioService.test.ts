...
describe('capacityScenarioService', () => {
  ...
  describe('update', () => {
    ...
    test('hoursPerPerson のみの更新ができる', async () => {
      const updatedRow = { ...sampleRow, hours_per_person: 180.00 }
      mockCapacityScenarioData.update.mockResolvedValue(updatedRow)

      const result = await capacityScenarioService.update(1, {
        hoursPerPerson: 180.00,
      })

      expect(result.hoursPerPerson).toBe(180.00)
  ...
  describe('calculate', () => {
    ...
    test('正常系: 人員計画データからキャパシティを計算して返す', async () => {
      mockCapacityScenarioData.findById.mockResolvedValue(sampleRow)
      mockMonthlyHeadcountPlanData.headcountPlanCaseExists.mockResolvedValue(true)
      mockMonthlyHeadcountPlanData.findForCalculation.mockResolvedValue(sampleHeadcountRows)
      ...
      expect(result.hoursPerPerson).toBe(128.00)
      expect(result.items).toHaveLength(2)
      expect(result.items[0].capacity).toBe(1280.00)
      expect(result.items[1].capacity).toBe(1536.00)
    })
    ...
    test('計算式 headcount × hoursPerPerson が正しい', async () => {
      ...
      expect(mockMonthlyCapacityData.bulkUpsert).toHaveBeenCalledWith(
        1,
        [
          { businessUnitCode: 'PLANT', yearMonth: '202601', capacity: 10 * 128.00 },
          { businessUnitCode: 'PLANT', yearMonth: '202602', capacity: 12 * 128.00 },
        ],
      )
    })
    ...
    test('businessUnitCodes を findForCalculation に渡す', async () => {
      ...
      expect(mockMonthlyHeadcountPlanData.findForCalculation).toHaveBeenCalledWith({
        headcountPlanCaseId: 1,
        businessUnitCodes: ['PLANT'],
    ...
    test('シナリオが存在しない場合 HTTPException(404) を投げる', async () => {
      ...
      try {
        await capacityScenarioService.calculate(999, { headcountPlanCaseId: 1 })
      } catch (e) {
        expect(e).toBeInstanceOf(HTTPException)
        expect((e as HTTPException).status).toBe(404)
    ...
    test('人員計画ケースが存在しない場合 HTTPException(404) を投げる', async () => {
      mockCapacityScenarioData.findById.mockResolvedValue(sampleRow)
      mockMonthlyHeadcountPlanData.headcountPlanCaseExists.mockResolvedValue(false)
      ...
      try {
        await capacityScenarioService.calculate(1, { headcountPlanCaseId: 999 })
      } catch (e) {
        expect(e).toBeInstanceOf(HTTPException)
        expect((e as HTTPException).status).toBe(404)
