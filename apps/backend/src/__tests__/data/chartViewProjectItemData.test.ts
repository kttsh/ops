...
describe('chartViewProjectItemData', () => {
  ...
  describe('update', () => {
    test('動的SET句を生成して更新する', async () => {
      ...
      const result = await chartViewProjectItemData.update(1, { displayOrder: 5 })
    ...
    test('projectCaseId のみ更新する場合', async () => {
      ...
      const result = await chartViewProjectItemData.update(1, { projectCaseId: 15 })
    ...
    test('isVisible のみ更新する場合', async () => {
      ...
      const result = await chartViewProjectItemData.update(1, { isVisible: false })

      expect(result).toEqual(updatedRow)
    })

    test('複数フィールドを同時に更新する場合', async () => {
      const updatedRow = { chart_view_project_item_id: 1, display_order: 3, is_visible: false }
      mockQuery.mockResolvedValueOnce({ rowsAffected: [1] })
      mockQuery.mockResolvedValueOnce({ recordset: [updatedRow] })
  ...
  describe('projectCaseBelongsToProject', () => {
    test('所属する場合 true を返す', async () => {
      ...
      const result = await chartViewProjectItemData.projectCaseBelongsToProject(12, 5)
    ...
    test('所属しない場合 false を返す', async () => {
      ...
      const result = await chartViewProjectItemData.projectCaseBelongsToProject(12, 999)
