...
describe('businessUnitData', () => {
  ...
  describe('findByCode', () => {
    test('指定コードのアクティブなレコードを返す', async () => {
      ...
      expect(mockInput).toHaveBeenCalledWith('code', expect.anything(), 'BU-001')
    })
  ...
  describe('findByCodeIncludingDeleted', () => {
    test('論理削除済みのレコードも含めて返す', async () => {
      ...
      const result = await businessUnitData.findByCodeIncludingDeleted('BU-002')

      expect(result).toEqual(deletedRow)
      expect(mockInput).toHaveBeenCalledWith('code', expect.anything(), 'BU-002')
    })

    test('存在しない場合は undefined を返す', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [] })

      const result = await businessUnitData.findByCodeIncludingDeleted('NON-EXISTENT')
  ...
  describe('create', () => {
    test('新規レコードを作成して作成結果を返す', async () => {
      ...
      expect(mockInput).toHaveBeenCalledWith('name', expect.anything(), 'ビジネスユニット1')
