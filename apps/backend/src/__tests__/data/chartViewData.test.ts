...
describe('chartViewData', () => {
  ...
  describe('create', () => {
    test('新規レコードを作成して OUTPUT で完全な行を返す', async () => {
      ...
      expect(result).toEqual(sampleRow)
      expect(mockInput).toHaveBeenCalledWith('viewName', expect.anything(), '2026年度全体ビュー')
      expect(mockInput).toHaveBeenCalledWith('chartType', expect.anything(), 'stacked-area')
      expect(mockInput).toHaveBeenCalledWith('startYearMonth', expect.anything(), '202604')
      expect(mockInput).toHaveBeenCalledWith('endYearMonth', expect.anything(), '202703')
      expect(mockInput).toHaveBeenCalledWith('isDefault', expect.anything(), true)
      expect(mockInput).toHaveBeenCalledWith('description', expect.anything(), '2026年度のプロジェクト工数積み上げ表示')
    })
  })
  ...
  describe('update', () => {
    test('指定フィールドのみ更新して findById で完全な行を返す', async () => {
      ...
      expect(mockInput).toHaveBeenCalledWith('id', expect.anything(), 1)
      expect(mockInput).toHaveBeenCalledWith('viewName', expect.anything(), '更新後ビュー')
    })
    ...
    test('複数フィールドを更新する', async () => {
      ...
      expect(mockInput).toHaveBeenCalledWith('viewName', expect.anything(), '更新ビュー')
      expect(mockInput).toHaveBeenCalledWith('chartType', expect.anything(), 'stacked-bar')
      expect(mockInput).toHaveBeenCalledWith('startYearMonth', expect.anything(), '202601')
      expect(mockInput).toHaveBeenCalledWith('endYearMonth', expect.anything(), '202612')
      expect(mockInput).toHaveBeenCalledWith('isDefault', expect.anything(), false)
