...
describe('headcountPlanCaseData', () => {
  let headcountPlanCaseData: typeof import('@/data/headcountPlanCaseData').headcountPlanCaseData

  beforeEach(async () => {
    vi.clearAllMocks()
    mockInput.mockReturnThis()
    const mod = await import('@/data/headcountPlanCaseData')
    headcountPlanCaseData = mod.headcountPlanCaseData
  })
  ...
  describe('findAll', () => {
    test('論理削除されていないレコードをページネーション付きで取得する', async () => {
      ...
      const result = await headcountPlanCaseData.findAll({
        page: 1,
        pageSize: 20,
    ...
    test('2ページ目を取得する場合、offset が正しく計算される', async () => {
      ...
      await headcountPlanCaseData.findAll({
        page: 2,
        pageSize: 10,
        includeDisabled: false,
      })
  ...
  describe('findById', () => {
    test('指定IDのアクティブなレコードを返す', async () => {
      ...
      const result = await headcountPlanCaseData.findById(1)
    ...
    test('存在しない場合は undefined を返す', async () => {
      ...
      const result = await headcountPlanCaseData.findById(999)
  ...
  describe('findByIdIncludingDeleted', () => {
    test('論理削除済みのレコードも含めて返す', async () => {
      ...
      const result = await headcountPlanCaseData.findByIdIncludingDeleted(2)
    ...
    test('存在しない場合は undefined を返す', async () => {
      ...
      const result = await headcountPlanCaseData.findByIdIncludingDeleted(999)
  ...
  describe('create', () => {
    test('新規レコードを作成して findById で完全な行を返す', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ recordset: [{ headcount_plan_case_id: 1 }] })
      // findById の結果
      mockQuery.mockResolvedValueOnce({ recordset: [sampleRow] })

      const result = await headcountPlanCaseData.create({
        caseName: '標準ケース',
        isPrimary: true,
        description: '標準的な人員計画',
      ...
      expect(mockInput).toHaveBeenCalledWith('description', expect.anything(), '標準的な人員計画')
  ...
  describe('update', () => {
    test('指定フィールドのみ更新して findById で完全な行を返す', async () => {
      ...
      const result = await headcountPlanCaseData.update(1, {
        caseName: '更新後ケース',
      })
    ...
    test('複数フィールドを更新する', async () => {
      ...
      await headcountPlanCaseData.update(1, {
        caseName: '更新ケース',
        isPrimary: false,
        description: '更新説明',
    ...
    test('存在しないレコードの場合は undefined を返す', async () => {
      ...
      const result = await headcountPlanCaseData.update(999, {
        caseName: '更新テスト',
      })
  ...
  describe('softDelete', () => {
    test('レコードを論理削除して結果を返す', async () => {
      ...
      const result = await headcountPlanCaseData.softDelete(1)
    ...
    test('存在しない/既に削除済みのレコードの場合は undefined を返す', async () => {
      ...
      const result = await headcountPlanCaseData.softDelete(999)
  ...
  describe('restore', () => {
    test('論理削除済みレコードを復元して findById で完全な行を返す', async () => {
      ...
      const result = await headcountPlanCaseData.restore(2)
    ...
    test('存在しない/削除されていないレコードの場合は undefined を返す', async () => {
      ...
      const result = await headcountPlanCaseData.restore(999)
  ...
  describe('hasReferences', () => {
    test('参照が存在する場合は true を返す', async () => {
      ...
      const result = await headcountPlanCaseData.hasReferences(1)
