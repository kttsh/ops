...
describe('createCapacityScenarioSchema - hoursPerPerson', () => {
  ...
  it('hoursPerPerson 省略時にデフォルト値 160.00 が適用される', () => {
    ...
    if (result.success) {
      expect(result.data.hoursPerPerson).toBe(160.00)
    }
  })

  it('hoursPerPerson を指定できる', () => {
    const result = createCapacityScenarioSchema.safeParse({ ...validData, hoursPerPerson: 128 })
    expect(result.success).toBe(true)
    if (result.success) {
  ...
  it('hoursPerPerson = 0.01（最小境界値）を受け付ける', () => {
    const result = createCapacityScenarioSchema.safeParse({ ...validData, hoursPerPerson: 0.01 })
    expect(result.success).toBe(true)
  })

  it('hoursPerPerson = 744（最大境界値）を受け付ける', () => {
    const result = createCapacityScenarioSchema.safeParse({ ...validData, hoursPerPerson: 744 })
    expect(result.success).toBe(true)
  })

  it('hoursPerPerson = 0 を拒否する', () => {
    const result = createCapacityScenarioSchema.safeParse({ ...validData, hoursPerPerson: 0 })
    expect(result.success).toBe(false)
  })

  it('hoursPerPerson = -1 を拒否する', () => {
    const result = createCapacityScenarioSchema.safeParse({ ...validData, hoursPerPerson: -1 })
    expect(result.success).toBe(false)
  })

  it('hoursPerPerson = 744.01 を拒否する', () => {
    const result = createCapacityScenarioSchema.safeParse({ ...validData, hoursPerPerson: 744.01 })
    expect(result.success).toBe(false)
  })
})
...
describe('updateCapacityScenarioSchema - hoursPerPerson', () => {
  it('hoursPerPerson を省略できる', () => {
    const result = updateCapacityScenarioSchema.safeParse({ scenarioName: 'テスト' })
    expect(result.success).toBe(true)
    if (result.success) {
  ...
  it('hoursPerPerson を指定できる', () => {
    const result = updateCapacityScenarioSchema.safeParse({ hoursPerPerson: 180 })
    expect(result.success).toBe(true)
    if (result.success) {
  ...
  it('hoursPerPerson = 0 を拒否する', () => {
    const result = updateCapacityScenarioSchema.safeParse({ hoursPerPerson: 0 })
    expect(result.success).toBe(false)
  })

  it('hoursPerPerson = 744.01 を拒否する', () => {
    const result = updateCapacityScenarioSchema.safeParse({ hoursPerPerson: 744.01 })
    expect(result.success).toBe(false)
  })
})
...
describe('calculateCapacitySchema', () => {
  it('必須フィールドのみで有効', () => {
    const result = calculateCapacitySchema.safeParse({ headcountPlanCaseId: 1 })
    expect(result.success).toBe(true)
  })
  ...
  it('headcountPlanCaseId = 0 を拒否する', () => {
    const result = calculateCapacitySchema.safeParse({ headcountPlanCaseId: 0 })
    expect(result.success).toBe(false)
  })

  it('headcountPlanCaseId が小数の場合を拒否する', () => {
    const result = calculateCapacitySchema.safeParse({ headcountPlanCaseId: 1.5 })
    expect(result.success).toBe(false)
  })
