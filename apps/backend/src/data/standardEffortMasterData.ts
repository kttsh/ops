...
export const standardEffortMasterData = {
  async findAll(params: {
    ...
    if (params.projectTypeCode) {
      itemsRequest.input('projectTypeCode', sql.VarChar, params.projectTypeCode)
      countRequest.input('projectTypeCode', sql.VarChar, params.projectTypeCode)
    }
  ...
  async findByIdIncludingDeleted(id: number): Promise<StandardEffortMasterRow | undefined> {
    const pool = await getPool()
    const result = await pool
      .request()
      .input('id', sql.Int, id)
  ...
  async findWeightsByMasterId(masterId: number): Promise<StandardEffortWeightRow[]> {
    const pool = await getPool()
    const result = await pool
      .request()
