...
const SELECT_COLUMNS = `
  ...
  i.display_order, i.is_visible, i.color_code, i.created_at, i.updated_at,
...
export const chartViewProjectItemData = {
	...
	async create(data: {
		...
		const insertResult = await pool
			...
			.input("colorCode", sql.VarChar, data.color ?? null)
			.query<{ chart_view_project_item_id: number }>(
				`INSERT INTO chart_view_project_items (
           chart_view_id, project_id, project_case_id, display_order, is_visible, color_code
         )
         OUTPUT INSERTED.chart_view_project_item_id
         VALUES (
           @chartViewId, @projectId, @projectCaseId, @displayOrder, @isVisible, @colorCode
         )`,
			);
	...
	async update(
		...
		if (data.color !== undefined) {
			...
			request.input("colorCode", sql.VarChar, data.color);
		}
		...
		try {
			...
			for (const item of items) {
				...
				await request
					...
					.input("colorCode", sql.VarChar, item.color)
					.query(
						`MERGE chart_view_project_items AS target
             USING (SELECT @chartViewId AS chart_view_id, @projectId AS project_id, @projectCaseId AS project_case_id) AS source
             ON target.chart_view_id = source.chart_view_id
                AND target.project_id = source.project_id
                AND ISNULL(target.project_case_id, -1) = ISNULL(source.project_case_id, -1)
             WHEN MATCHED THEN
               UPDATE SET display_order = @displayOrder, is_visible = @isVisible, color_code = @colorCode, updated_at = GETDATE()
             WHEN NOT MATCHED THEN
               INSERT (chart_view_id, project_id, project_case_id, display_order, is_visible, color_code)
               VALUES (@chartViewId, @projectId, @projectCaseId, @displayOrder, @isVisible, @colorCode);`,
					);
			}
			...
			if (items.length > 0) {
				...
				const keepConditions = items.map((item, i) => {
					deleteRequest.input(`pid${i}`, sql.Int, item.projectId);
					deleteRequest.input(`pcid${i}`, sql.Int, item.projectCaseId);
					return `(project_id = @pid${i} AND ISNULL(project_case_id, -1) = ISNULL(@pcid${i}, -1))`;
				});
           ...
           WHERE chart_view_id = @chartViewId
             AND NOT (${keepConditions.join(" OR ")})`,
				...
				deleteRequest.input("chartViewId", sql.Int, chartViewId);
				await deleteRequest.query(
					`DELETE FROM chart_view_project_items WHERE chart_view_id = @chartViewId`,
				);
			}
