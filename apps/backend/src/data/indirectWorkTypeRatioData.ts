...
export const indirectWorkTypeRatioData = {
  async findAll(indirectWorkCaseId: number): Promise<IndirectWorkTypeRatioRow[]> {
  ...
  async findById(indirectWorkTypeRatioId: number): Promise<IndirectWorkTypeRatioRow | undefined> {
    const pool = await getPool()
    const result = await pool
      .request()
