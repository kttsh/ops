...
export const indirectWorkCaseData = {
	async findAll(params: {
		...
		if (!params.includeDisabled) {
			conditions.push("iwc.deleted_at IS NULL");
		}
		if (params.businessUnitCode) {
			conditions.push("iwc.business_unit_code = @businessUnitCode");
		}
		const whereClause =
		...
		const itemsResult = await itemsRequest.query<IndirectWorkCaseRow>(
			`SELECT ${BASE_SELECT}
         ${FROM_WITH_JOIN}
         ${whereClause}
