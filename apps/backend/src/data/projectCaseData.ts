...
export const projectCaseData = {
  async findAll(params: {
    ...
    const deletedFilter = params.includeDisabled ? '' : 'AND pc.deleted_at IS NULL'
  ...
  async findByIdIncludingDeleted(projectCaseId: number): Promise<ProjectCaseRow | undefined> {
    const pool = await getPool()
    const result = await pool
      .request()
      .input('projectCaseId', sql.Int, projectCaseId)
