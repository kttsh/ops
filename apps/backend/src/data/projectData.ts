...
import type { ProjectRow, CreateProject } from '@/types/project'
...
export const projectData = {
  async findAll(params: {
    ...
    const countWhereClause = countWhereClauses.length > 0
  ...
  async update(id: number, data: Record<string, unknown>): Promise<ProjectRow | undefined> {
    const pool = await getPool()
    const setClauses: string[] = ['updated_at = GETDATE()']
    const request = pool.request().input('id', sql.Int, id)

    const fieldMap: Record<string, { sqlType: sql.ISqlTypeFactory; column: string }> = {
