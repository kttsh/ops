...
function transformProjectLoads(rows: ProjectLoadRow[]): ProjectLoadAggregation[] {
  ...
  for (const row of rows) {
    const key = row.projectTypeCode ?? '__null__'
...
function transformIndirectWorkLoads(rows: IndirectWorkLoadRow[]): IndirectWorkLoadAggregation[] {
  const groups = new Map<string, IndirectWorkLoadAggregation>()

  for (const row of rows) {
    const groupKey = `${row.indirectWorkCaseId}:${row.businessUnitCode}`
    const monthKey = `${groupKey}:${row.yearMonth}`
    ...
    if (row.workTypeCode !== null && row.ratio !== null && row.typeManhour !== null) {
      monthEntry.breakdown.push({
        workTypeCode: row.workTypeCode,
        workTypeName: row.workTypeName!,
        manhour: row.typeManhour,
      })
      monthEntry.breakdownCoverage = Math.round(
        (monthEntry.breakdownCoverage + row.ratio) * 100,
      ) / 100
    }
  }
...
function transformCapacities(rows: CapacityRow[]): CapacityAggregation[] {
  const groups = new Map<number, { scenarioName: string; monthlyMap: Map<string, number> }>()
  ...
  return Array.from(groups.entries()).map(([id, group]) => ({
    ...
    monthly: Array.from(group.monthlyMap.entries()).map(([yearMonth, capacity]) => ({
      yearMonth,
      capacity,
    })),
  }))
