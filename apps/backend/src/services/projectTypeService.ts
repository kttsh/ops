...
import type { ProjectType, CreateProjectType, UpdateProjectType } from '@/types/projectType'
...
export const projectTypeService = {
  ...
  async create(data: CreateProjectType): Promise<ProjectType> {
    const existing = await projectTypeData.findByCodeIncludingDeleted(data.projectTypeCode)
    if (existing) {
