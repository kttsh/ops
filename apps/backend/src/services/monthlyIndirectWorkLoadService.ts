...
export const monthlyIndirectWorkLoadService = {
  async findAll(indirectWorkCaseId: number): Promise<MonthlyIndirectWorkLoad[]> {
    const exists = await monthlyIndirectWorkLoadData.indirectWorkCaseExists(indirectWorkCaseId)
    if (!exists) {
  ...
  async findById(indirectWorkCaseId: number, monthlyIndirectWorkLoadId: number): Promise<MonthlyIndirectWorkLoad> {
    const row = await monthlyIndirectWorkLoadData.findById(monthlyIndirectWorkLoadId)
    if (!row) {
      throw new HTTPException(404, {
  ...
  async create(indirectWorkCaseId: number, data: CreateMonthlyIndirectWorkLoad): Promise<MonthlyIndirectWorkLoad> {
    const caseExists = await monthlyIndirectWorkLoadData.indirectWorkCaseExists(indirectWorkCaseId)
    if (!caseExists) {
      throw new HTTPException(404, {
        message: `Indirect work case with ID '${indirectWorkCaseId}' not found`,
      })
    ...
    const buExists = await monthlyIndirectWorkLoadData.businessUnitExists(data.businessUnitCode)
    if (!buExists) {
  ...
  async update(
    ...
    const existing = await monthlyIndirectWorkLoadData.findById(monthlyIndirectWorkLoadId)
    if (!existing) {
      throw new HTTPException(404, {
        message: `Monthly indirect work load with ID '${monthlyIndirectWorkLoadId}' not found`,
      })
    ...
    const checkBusinessUnitCode = data.businessUnitCode ?? existing.business_unit_code
    ...
    const updated = await monthlyIndirectWorkLoadData.update(monthlyIndirectWorkLoadId, data)
    if (!updated) {
      throw new HTTPException(404, {
        message: `Monthly indirect work load with ID '${monthlyIndirectWorkLoadId}' not found`,
      })
  ...
  async delete(indirectWorkCaseId: number, monthlyIndirectWorkLoadId: number): Promise<void> {
    const existing = await monthlyIndirectWorkLoadData.findById(monthlyIndirectWorkLoadId)
    if (!existing) {
      throw new HTTPException(404, {
        message: `Monthly indirect work load with ID '${monthlyIndirectWorkLoadId}' not found`,
  ...
  async bulkUpsert(
    ...
    const allBuExist = await monthlyIndirectWorkLoadData.businessUnitsExist(businessUnitCodes)
    if (!allBuExist) {
      throw new HTTPException(422, {
    ...
    const rows = await monthlyIndirectWorkLoadData.bulkUpsert(indirectWorkCaseId, data.items)
    return rows.map(toMonthlyIndirectWorkLoadResponse)
  },
}
