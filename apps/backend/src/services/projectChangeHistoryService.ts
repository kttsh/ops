...
import type { ProjectChangeHistory, CreateProjectChangeHistory } from '@/types/projectChangeHistory'
...
export const projectChangeHistoryService = {
  ...
  async findById(projectId: number, projectChangeHistoryId: number): Promise<ProjectChangeHistory> {
  ...
  async create(projectId: number, data: CreateProjectChangeHistory): Promise<ProjectChangeHistory> {
    const exists = await projectChangeHistoryData.projectExists(projectId)
    if (!exists) {
      throw new HTTPException(404, {
        message: `Project with ID '${projectId}' not found`,
  ...
  async delete(projectId: number, projectChangeHistoryId: number): Promise<void> {
    const row = await projectChangeHistoryData.findById(projectChangeHistoryId)
    if (!row) {
      throw new HTTPException(404, {
        message: `Project change history with ID '${projectChangeHistoryId}' not found`,
