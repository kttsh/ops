...
export const monthlyHeadcountPlanService = {
  async findAll(headcountPlanCaseId: number, businessUnitCode?: string): Promise<MonthlyHeadcountPlan[]> {
    const exists = await monthlyHeadcountPlanData.headcountPlanCaseExists(headcountPlanCaseId)
    if (!exists) {
    ...
    const rows = await monthlyHeadcountPlanData.findAll(headcountPlanCaseId, businessUnitCode)
    return rows.map(toMonthlyHeadcountPlanResponse)
  },

  async findById(headcountPlanCaseId: number, monthlyHeadcountPlanId: number): Promise<MonthlyHeadcountPlan> {
  ...
  async create(headcountPlanCaseId: number, data: CreateMonthlyHeadcountPlan): Promise<MonthlyHeadcountPlan> {
    const caseExists = await monthlyHeadcountPlanData.headcountPlanCaseExists(headcountPlanCaseId)
    if (!caseExists) {
      throw new HTTPException(404, {
        message: `Headcount plan case with ID '${headcountPlanCaseId}' not found`,
      })
    ...
    const buExists = await monthlyHeadcountPlanData.businessUnitExists(data.businessUnitCode)
    if (!buExists) {
      throw new HTTPException(404, {
  ...
  async update(
    ...
    const existing = await monthlyHeadcountPlanData.findById(monthlyHeadcountPlanId)
    if (!existing) {
      throw new HTTPException(404, {
        message: `Monthly headcount plan with ID '${monthlyHeadcountPlanId}' not found`,
      })
    ...
    const updated = await monthlyHeadcountPlanData.update(monthlyHeadcountPlanId, data)
    if (!updated) {
      throw new HTTPException(404, {
        message: `Monthly headcount plan with ID '${monthlyHeadcountPlanId}' not found`,
      })
  ...
  async delete(headcountPlanCaseId: number, monthlyHeadcountPlanId: number): Promise<void> {
    const existing = await monthlyHeadcountPlanData.findById(monthlyHeadcountPlanId)
    if (!existing) {
      throw new HTTPException(404, {
        message: `Monthly headcount plan with ID '${monthlyHeadcountPlanId}' not found`,
  ...
  async bulkUpsert(headcountPlanCaseId: number, data: BulkUpsertMonthlyHeadcountPlan): Promise<MonthlyHeadcountPlan[]> {
    const caseExists = await monthlyHeadcountPlanData.headcountPlanCaseExists(headcountPlanCaseId)
    if (!caseExists) {
      throw new HTTPException(404, {
        message: `Headcount plan case with ID '${headcountPlanCaseId}' not found`,
    ...
    const keys = data.items.map((item) => `${item.businessUnitCode}:${item.yearMonth}`)
    ...
    const businessUnitCodes = [...new Set(data.items.map((item) => item.businessUnitCode))]
    const busExist = await monthlyHeadcountPlanData.businessUnitsExist(businessUnitCodes)
    if (!busExist) {
      throw new HTTPException(404, {
    ...
    const rows = await monthlyHeadcountPlanData.bulkUpsert(headcountPlanCaseId, data.items)
    return rows.map(toMonthlyHeadcountPlanResponse)
  },
}
