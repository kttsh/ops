...
import type {
	BulkUpsertMonthlyCapacity,
	CreateMonthlyCapacity,
	MonthlyCapacity,
...
export const monthlyCapacityService = {
	async findAll(capacityScenarioId: number): Promise<MonthlyCapacity[]> {
		const exists =
			await monthlyCapacityData.capacityScenarioExists(capacityScenarioId);
		if (!exists) {
	...
	async create(
		...
		const buExists = await monthlyCapacityData.businessUnitExists(
	...
	async bulkUpsert(
		...
		const keys = data.items.map(
			(item) => `${item.businessUnitCode}_${item.yearMonth}`,
		);
		...
		const buAllExist =
			await monthlyCapacityData.businessUnitsExist(uniqueBuCodes);
		if (!buAllExist) {
			throw new HTTPException(422, {
		...
		const rows = await monthlyCapacityData.bulkUpsert(
			capacityScenarioId,
			data.items,
		);
		return rows.map(toMonthlyCapacityResponse);
