...
import { toSummaryResponse, toDetailResponse } from '@/transform/standardEffortMasterTransform'
import type {
...
export const standardEffortMasterService = {
  ...
  async create(data: CreateStandardEffortMaster): Promise<StandardEffortMasterDetail> {
    ...
    const weights = await standardEffortMasterData.findWeightsByMasterId(created.standard_effort_id)
    return toDetailResponse(created, weights)
  },

  async update(id: number, data: UpdateStandardEffortMaster): Promise<StandardEffortMasterDetail> {
  ...
  async restore(id: number): Promise<StandardEffortMasterDetail> {
    const existing = await standardEffortMasterData.findByIdIncludingDeleted(id)
