...
import type { ProjectCase, CreateProjectCase, UpdateProjectCase } from '@/types/projectCase'
...
export const projectCaseService = {
  ...
  async findById(projectId: number, projectCaseId: number): Promise<ProjectCase> {
  ...
  async create(projectId: number, data: CreateProjectCase): Promise<ProjectCase> {
    ...
    if (data.calculationType === 'STANDARD' && !data.standardEffortId) {
      throw new HTTPException(422, {
        message: 'standardEffortId is required when calculationType is STANDARD',
      })
    }

    if (data.standardEffortId) {
      const seExists = await projectCaseData.standardEffortExists(data.standardEffortId)
      if (!seExists) {
        throw new HTTPException(422, {
  ...
  async update(projectId: number, projectCaseId: number, data: UpdateProjectCase): Promise<ProjectCase> {
    ...
    const effectiveCalculationType = data.calculationType ?? existing.calculation_type
    const effectiveStandardEffortId = data.standardEffortId !== undefined
  ...
  async restore(projectId: number, projectCaseId: number): Promise<ProjectCase> {
    const existing = await projectCaseData.findByIdIncludingDeleted(projectCaseId)
    if (!existing) {
      throw new HTTPException(404, {
        message: `Project case with ID '${projectCaseId}' not found`,
      })
