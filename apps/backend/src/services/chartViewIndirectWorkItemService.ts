...
export const chartViewIndirectWorkItemService = {
	async findAll(chartViewId: number): Promise<ChartViewIndirectWorkItem[]> {
		const exists =
			await chartViewIndirectWorkItemData.chartViewExists(chartViewId);
		if (!exists) {
	...
	async findById(
		chartViewId: number,
		id: number,
	): Promise<ChartViewIndirectWorkItem> {
	...
	async create(
		chartViewId: number,
		data: CreateChartViewIndirectWorkItem,
	): Promise<ChartViewIndirectWorkItem> {
		const exists =
			await chartViewIndirectWorkItemData.chartViewExists(chartViewId);
		if (!exists) {
		...
		const caseExists =
			await chartViewIndirectWorkItemData.indirectWorkCaseExists(
				data.indirectWorkCaseId,
			);
		if (!caseExists) {
		...
		const duplicate = await chartViewIndirectWorkItemData.duplicateExists(
			chartViewId,
			data.indirectWorkCaseId,
		);
		if (duplicate) {
	...
	async update(
		chartViewId: number,
		id: number,
		data: UpdateChartViewIndirectWorkItem,
	): Promise<ChartViewIndirectWorkItem> {
