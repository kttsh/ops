...
export const chartColorSettingService = {
  ...
  async create(data: CreateChartColorSetting): Promise<ChartColorSetting> {
    const existing = await chartColorSettingData.findByTarget(data.targetType, data.targetId)
    if (existing) {
  ...
  async update(id: number, data: UpdateChartColorSetting): Promise<ChartColorSetting> {
