...
export const indirectWorkTypeRatioService = {
  async findAll(indirectWorkCaseId: number): Promise<IndirectWorkTypeRatio[]> {
    const exists = await indirectWorkTypeRatioData.indirectWorkCaseExists(indirectWorkCaseId)
    if (!exists) {
  ...
  async findById(indirectWorkCaseId: number, indirectWorkTypeRatioId: number): Promise<IndirectWorkTypeRatio> {
    const row = await indirectWorkTypeRatioData.findById(indirectWorkTypeRatioId)
    if (!row) {
      throw new HTTPException(404, {
  ...
  async create(indirectWorkCaseId: number, data: CreateIndirectWorkTypeRatio): Promise<IndirectWorkTypeRatio> {
    const exists = await indirectWorkTypeRatioData.indirectWorkCaseExists(indirectWorkCaseId)
    if (!exists) {
      throw new HTTPException(404, {
        message: `Indirect work case with ID '${indirectWorkCaseId}' not found`,
    ...
    const workTypeExists = await indirectWorkTypeRatioData.workTypeExists(data.workTypeCode)
    if (!workTypeExists) {
  ...
  async update(
    ...
    const existing = await indirectWorkTypeRatioData.findById(indirectWorkTypeRatioId)
    if (!existing) {
      throw new HTTPException(404, {
        message: `Indirect work type ratio with ID '${indirectWorkTypeRatioId}' not found`,
      })
    ...
    const updated = await indirectWorkTypeRatioData.update(indirectWorkTypeRatioId, data)
    if (!updated) {
      throw new HTTPException(404, {
        message: `Indirect work type ratio with ID '${indirectWorkTypeRatioId}' not found`,
      })
  ...
  async delete(indirectWorkCaseId: number, indirectWorkTypeRatioId: number): Promise<void> {
    const existing = await indirectWorkTypeRatioData.findById(indirectWorkTypeRatioId)
    if (!existing) {
      throw new HTTPException(404, {
        message: `Indirect work type ratio with ID '${indirectWorkTypeRatioId}' not found`,
  ...
  async bulkUpsert(indirectWorkCaseId: number, data: BulkUpsertIndirectWorkTypeRatio): Promise<IndirectWorkTypeRatio[]> {
    const exists = await indirectWorkTypeRatioData.indirectWorkCaseExists(indirectWorkCaseId)
    if (!exists) {
      throw new HTTPException(404, {
        message: `Indirect work case with ID '${indirectWorkCaseId}' not found`,
    ...
    const keys = data.items.map((item) => `${item.workTypeCode}:${item.fiscalYear}`)
    ...
    const rows = await indirectWorkTypeRatioData.bulkUpsert(indirectWorkCaseId, data.items)
    return rows.map(toIndirectWorkTypeRatioResponse)
  },
}
