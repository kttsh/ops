...
const app = new Hono()
  ...
  .get(
    '/',
    validate('query', chartViewListQuerySchema),
    async (c) => {
  ...
  .post(
    '/',
    validate('json', createChartViewSchema),
    async (c) => {
      const body = c.req.valid('json')
      const created = await chartViewService.create(body)
      c.header('Location', `/chart-views/${created.chartViewId}`)
  ...
  .put(
    '/:id',
    validate('json', updateChartViewSchema),
    async (c) => {
      const id = Number(c.req.param('id'))
      const body = c.req.valid('json')
