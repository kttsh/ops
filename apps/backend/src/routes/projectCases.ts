...
const app = new Hono()
  ...
  .get(
    '/',
    validate('query', projectCaseListQuerySchema),
    async (c) => {
  ...
  .get('/:projectCaseId', async (c) => {
    ...
    const projectCase = await projectCaseService.findById(projectId, projectCaseId)
    return c.json({ data: projectCase }, 200)
  })
  // POST / - 新規作成
  .post(
    '/',
    validate('json', createProjectCaseSchema),
    async (c) => {
      const projectId = parseIntParam(c.req.param('projectId'), 'projectId')
      const body = c.req.valid('json')
      const created = await projectCaseService.create(projectId, body)
      c.header('Location', `/projects/${projectId}/project-cases/${created.projectCaseId}`)
  ...
  .put(
    ...
    async (c) => {
      ...
      const projectCaseId = parseIntParam(c.req.param('projectCaseId'), 'projectCaseId')
      const body = c.req.valid('json')
      const updated = await projectCaseService.update(projectId, projectCaseId, body)
