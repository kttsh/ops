...
const app = new Hono()
	...
	.put("/bulk", validate("json", bulkUpsertProjectLoadSchema), async (c) => {
		const projectCaseId = parseIntParam(
			c.req.param("projectCaseId"),
			"projectCaseId",
		);
	...
	.get("/:projectLoadId", async (c) => {
		...
		const projectLoadId = parseIntParam(
			c.req.param("projectLoadId"),
			"projectLoadId",
		);
		const projectLoad = await projectLoadService.findById(
			projectCaseId,
			projectLoadId,
		);
	...
	// POST / - 新規作成
	.post("/", validate("json", createProjectLoadSchema), async (c) => {
		const projectCaseId = parseIntParam(
			c.req.param("projectCaseId"),
			"projectCaseId",
		);
		...
		c.header(
			"Location",
			`/project-cases/${projectCaseId}/project-loads/${created.projectLoadId}`,
		);
	...
	.put(
		...
		async (c) => {
			...
			const body = c.req.valid("json");
			const updated = await projectLoadService.update(
				projectCaseId,
				projectLoadId,
				body,
			);
