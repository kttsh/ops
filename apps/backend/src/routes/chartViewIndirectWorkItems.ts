...
const app = new Hono()
	...
	.get("/:chartViewIndirectWorkItemId", async (c) => {
		...
		const id = parseIntParam(
			c.req.param("chartViewIndirectWorkItemId"),
			"chartViewIndirectWorkItemId",
		);
		const item = await chartViewIndirectWorkItemService.findById(
			chartViewId,
			id,
		);
	...
	.post(
		...
		async (c) => {
			...
			const created = await chartViewIndirectWorkItemService.create(
				chartViewId,
				body,
			);
			c.header(
				"Location",
				`/chart-views/${chartViewId}/indirect-work-items/${created.chartViewIndirectWorkItemId}`,
			);
	...
	.put(
		...
		async (c) => {
			...
			const body = c.req.valid("json");
			const updated = await chartViewIndirectWorkItemService.update(
				chartViewId,
				id,
				body,
			);
