...
const app = new Hono()
	// GET / - 一覧取得
	.get("/", validate("query", headcountPlanCaseListQuerySchema), async (c) => {
	...
	// POST / - 新規作成
	.post("/", validate("json", createHeadcountPlanCaseSchema), async (c) => {
		const body = c.req.valid("json");
		const created = await headcountPlanCaseService.create(body);
		c.header(
			"Location",
			`/headcount-plan-cases/${created.headcountPlanCaseId}`,
		);
		return c.json({ data: created }, 201);
	})
	// PUT /:id - 更新
	.put("/:id", validate("json", updateHeadcountPlanCaseSchema), async (c) => {
		const id = Number(c.req.param("id"));
		const body = c.req.valid("json");
