...
const app = new Hono()
	...
	.put(
		"/bulk",
		validate("json", bulkUpsertChartViewProjectItemSchema),
		async (c) => {
			const chartViewId = parseIntParam(
				c.req.param("chartViewId"),
				"chartViewId",
			...
			const items = await chartViewProjectItemService.bulkUpsert(
				chartViewId,
				body,
			);
			return c.json({ data: items }, 200);
	...
	.put(
		...
		async (c) => {
			...
			const body = c.req.valid("json");
			const items = await chartViewProjectItemService.updateDisplayOrder(
				chartViewId,
				body,
			);
			return c.json({ data: items }, 200);
	...
	// POST / - 新規作成
	.post("/", validate("json", createChartViewProjectItemSchema), async (c) => {
		const chartViewId = parseIntParam(
			c.req.param("chartViewId"),
			"chartViewId",
		);
		...
		c.header(
			"Location",
			`/chart-views/${chartViewId}/project-items/${created.chartViewProjectItemId}`,
		);
	...
	.put(
		...
		async (c) => {
			...
			const id = parseIntParam(c.req.param("id"), "id");
			const body = c.req.valid("json");
			const updated = await chartViewProjectItemService.update(
				chartViewId,
				id,
				body,
			);
