...
const app = new Hono()
	// GET / - 一覧取得
	.get("/", async (c) => {
		const capacityScenarioId = parseIntParam(
			c.req.param("capacityScenarioId"),
			"capacityScenarioId",
		);
	...
	.put(
		...
		async (c) => {
			...
			const items = await monthlyCapacityService.bulkUpsert(
				capacityScenarioId,
				body,
			);
			return c.json({ data: items }, 200);
	...
	.get("/:monthlyCapacityId", async (c) => {
		...
		const monthlyCapacityId = parseIntParam(
			c.req.param("monthlyCapacityId"),
			"monthlyCapacityId",
		);
		const monthlyCapacity = await monthlyCapacityService.findById(
			capacityScenarioId,
			monthlyCapacityId,
		);
	...
	// POST / - 新規作成
	.post("/", validate("json", createMonthlyCapacitySchema), async (c) => {
		const capacityScenarioId = parseIntParam(
			c.req.param("capacityScenarioId"),
			"capacityScenarioId",
		);
		const body = c.req.valid("json");
		const created = await monthlyCapacityService.create(
			capacityScenarioId,
			body,
		);
		c.header(
			"Location",
			`/capacity-scenarios/${capacityScenarioId}/monthly-capacities/${created.monthlyCapacityId}`,
		);
	...
	.put(
		...
		async (c) => {
			...
			const body = c.req.valid("json");
			const updated = await monthlyCapacityService.update(
				capacityScenarioId,
				monthlyCapacityId,
				body,
			);
