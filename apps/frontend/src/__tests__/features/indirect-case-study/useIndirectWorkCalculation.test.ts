...
import type { IndirectWorkTypeRatio } from "@/features/indirect-case-study/types";
...
describe("間接工数計算ロジック", () => {
	...
	it("複数年度にまたがるデータの計算", () => {
		...
		const apr2025 = Math.round(
			1600 * getTotalRatioForFiscalYear(ratios, getFiscalYear("202504")),
		);
		expect(apr2025).toBe(240);

		// 202601 (FY2025): 1600 * 0.15 = 240
		const jan2026 = Math.round(
			1600 * getTotalRatioForFiscalYear(ratios, getFiscalYear("202601")),
		);
		expect(jan2026).toBe(240);

		// 202604 (FY2026): 1600 * 0.12 = 192
		const apr2026 = Math.round(
			1600 * getTotalRatioForFiscalYear(ratios, getFiscalYear("202604")),
		);
