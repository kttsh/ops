...
describe('fetchBusinessUnits', () => {
  it('正常にビジネスユニット一覧を取得する', async () => {
    const mockData = {
      data: [{ businessUnitCode: 'BU-001', name: 'テスト', displayOrder: 0, createdAt: '2024-01-01', updatedAt: '2024-01-01' }],
    ...
    const result = await fetchBusinessUnits({ page: 1, pageSize: 20, includeDisabled: false })
  ...
  it('エラーレスポンスでApiErrorをthrowする', async () => {
    ...
    await expect(fetchBusinessUnits({ page: 1, pageSize: 20, includeDisabled: false })).rejects.toThrow(ApiError)
  })
})
...
describe('fetchBusinessUnit', () => {
  it('正常に単一のビジネスユニットを取得する', async () => {
    ...
    expect(result).toEqual(mockData)
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/business-units/BU-001'))
  })
...
describe('createBusinessUnit', () => {
  it('正常にビジネスユニットを作成する', async () => {
    const mockData = {
      data: { businessUnitCode: 'BU-NEW', name: '新規', displayOrder: 0, createdAt: '2024-01-01', updatedAt: '2024-01-01' },
    }
    mockFetch.mockResolvedValueOnce({
      ok: true,
      status: 201,
  ...
  it('409 ConflictでApiErrorをthrowする', async () => {
    ...
    try {
      await createBusinessUnit({ businessUnitCode: 'BU-EXISTING', name: '重複', displayOrder: 0 })
    } catch (err) {
      expect(err).toBeInstanceOf(ApiError)
...
describe('updateBusinessUnit', () => {
  it('正常にビジネスユニットを更新する', async () => {
    const mockData = {
      data: { businessUnitCode: 'BU-001', name: '更新済み', displayOrder: 5, createdAt: '2024-01-01', updatedAt: '2024-01-02' },
    }
    mockFetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
    ...
    const result = await updateBusinessUnit('BU-001', { name: '更新済み', displayOrder: 5 })
    expect(result).toEqual(mockData)
    expect(mockFetch).toHaveBeenCalledWith(
...
describe('restoreBusinessUnit', () => {
  it('正常にビジネスユニットを復元する', async () => {
    const mockData = {
      data: { businessUnitCode: 'BU-001', name: 'テスト', displayOrder: 0, createdAt: '2024-01-01', updatedAt: '2024-01-03' },
    }
    mockFetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
