...
export const Route = createFileRoute('/master/business-units/$businessUnitCode/edit')({
...
function BusinessUnitEditPage() {
  ...
  const { data, isLoading } = useQuery(businessUnitQueryOptions(businessUnitCode))
    ...
    try {
      ...
      if (err instanceof ApiError) {
        if (err.problemDetails.status === 404) {
          toast.error('ビジネスユニットが見つかりません', { duration: Infinity })
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center py-16 space-y-4">
        ...
        <Link to="/master/business-units" className="text-sm text-primary hover:underline">
          一覧に戻る
        </Link>
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-bold tracking-tight">ビジネスユニット 編集</h2>
