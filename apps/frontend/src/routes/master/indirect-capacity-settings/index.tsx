...
export const Route = createFileRoute('/master/indirect-capacity-settings/')({
...
function IndirectCapacitySettingsPage() {
  ...
  const { data: buData, isLoading: buLoading } = useQuery(businessUnitsQueryOptions())
  const { data: wtData } = useQuery(workTypesQueryOptions({ includeDisabled: false }))
  ...
  const selectedBu = useMemo(() => {
    if (search.bu && businessUnits.some((bu) => bu.businessUnitCode === search.bu)) {
      return search.bu
    }
  ...
  return (
    <div className="flex h-full flex-col">
          ...
          <Select value={selectedBu} onValueChange={handleBuChange}>
            ...
            <SelectContent>
              {businessUnits.map((bu) => (
                <SelectItem key={bu.businessUnitCode} value={bu.businessUnitCode}>
                  {bu.name}
                </SelectItem>
              ))}
            </SelectContent>
      ...
      <UnsavedChangesDialog
        ...
        isSaving={page.isSavingResults || page.isSavingHeadcount || page.isSavingRatios}
      />
    </div>
  )
}
