...
export const Route = createFileRoute('/master/projects/$projectId/')({
  ...
  notFoundComponent: () => (
    <div className="flex flex-col items-center justify-center py-16 space-y-4">
      ...
      <Link to="/master/projects" className="text-sm text-primary hover:underline">
        一覧に戻る
      </Link>
    </div>
  ),
    ...
    try {
      ...
      if (err instanceof ApiError) {
        if (err.problemDetails.status === 409) {
          toast.error('この案件は他のデータから参照されているため削除できません', {
            duration: Infinity,
          })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border shadow-sm p-6 space-y-4">
        ...
        <DetailRow label="開始年月" value={formatYearMonth(project.startYearMonth)} />
