  ...
  return (
    ...
    >
      <form.Field
        name="workTypeCode"
        validators={{
          onChange: mode === 'create' ? createWorkTypeSchema.shape.workTypeCode : undefined,
        }}
      >
        {(field) => (
      ...
      <form.Field
        ...
        validators={{
          onChange: ({ value }) => {
            if (typeof value !== 'number' || !Number.isInteger(value)) return '表示順は整数で入力してください'
      ...
      <form.Field
        ...
        validators={{
          onChange: ({ value }) => {
            ...
            if (!/^#[0-9A-Fa-f]{6}$/.test(value)) return 'カラーコードは #RRGGBB 形式で入力してください'
            return undefined
          },
        }}
      >
