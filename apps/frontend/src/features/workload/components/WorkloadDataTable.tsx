...
export function WorkloadDataTable({
	...
	const columns = useMemo<ColumnDef<TableRow>[]>(() => {
		const fixed: ColumnDef<TableRow>[] = [
			{
				...
				cell: ({ row }) => {
					...
					const hasSubRows =
						original.rowType === "project" &&
						(original.subRows?.length ?? 0) > 0;
					...
					return (
						<div className="flex items-center gap-1">
							...
							<span
								className={cn(
									"truncate",
									isDetail && "pl-2 text-muted-foreground",
								)}
							>
								{original.name}
							</span>
		...
		const monthCols: ColumnDef<TableRow>[] = Array.from(
			{ length: 12 },
			(_, i) => {
				...
				return {
					...
					cell: ({ getValue }) =>
						formatCellValue(getValue() as number | undefined),
				};
			},
		);
	...
	return (
		<div className="flex flex-col gap-3">
			...
			{/* フッター情報 */}
			<div className="text-xs text-muted-foreground">{flatRows.length} 行</div>
		</div>
	);
}
