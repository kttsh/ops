...
export const mockTableRows: TableRow[] = [
  {
    ...
    name: 'キャパシティ',
    total: 9600,
    monthly: Object.fromEntries(months.map((m) => [`${m.yearMonth.slice(0, 4)}_${m.yearMonth.slice(4)}`, 800])),
  },
  {
    id: 'project-1',
    rowType: 'project',
    ...
    projectTypeName: '設計',
    total: 4800,
    monthly: Object.fromEntries(months.map((m) => [`${m.yearMonth.slice(0, 4)}_${m.yearMonth.slice(4)}`, 400])),
  },
  {
    id: 'project-2',
    rowType: 'project',
    ...
    total: 3000,
    monthly: Object.fromEntries(months.map((m) => [`${m.yearMonth.slice(0, 4)}_${m.yearMonth.slice(4)}`, 250])),
  },
]
