...
import type { CreateProjectTypeInput, UpdateProjectTypeInput } from '@/features/project-types/types'
...
export function useUpdateProjectType(code: string) {
  ...
  return useMutation({
    mutationFn: (input: UpdateProjectTypeInput) => updateProjectType(code, input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: projectTypeKeys.lists() })
