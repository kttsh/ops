...
interface IndirectWorkCaseListProps {
  items: IndirectWorkCase[]
...
export function IndirectWorkCaseList({
  items,
  selectedId,
  onSelect,
  businessUnitCode,
...
}: IndirectWorkCaseListProps) {
  const [formOpen, setFormOpen] = useState(false)
  const [formMode, setFormMode] = useState<'create' | 'edit'>('create')
  const [editTarget, setEditTarget] = useState<IndirectWorkCase | null>(null)
  const [deleteTargetId, setDeleteTargetId] = useState<number | null>(null)

  const createMutation = useCreateIndirectWorkCase()
  const updateMutation = useUpdateIndirectWorkCase()
  const deleteMutation = useDeleteIndirectWorkCase()
  const restoreMutation = useRestoreIndirectWorkCase()
  ...
  const handleEdit = (item: IndirectWorkCase) => {
    setFormMode('edit')
    setEditTarget(item)
    setFormOpen(true)
  }
  ...
  const handleSubmit = async (values: { caseName: string; description: string; isPrimary: boolean }) => {
    if (formMode === 'create') {
      ...
      await updateMutation.mutateAsync({
        id: editTarget.indirectWorkCaseId,
        input: values,
      })
    }
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold">間接作業ケース</h3>
      ...
      <div className="flex items-center gap-2">
        <Switch
          id="iwc-include-disabled"
          checked={includeDisabled}
          onCheckedChange={onIncludeDisabledChange}
        />
        <Label htmlFor="iwc-include-disabled" className="text-xs text-muted-foreground">
          削除済みを含む
        </Label>
      </div>
            ...
            >
              ...
              {isDeleted ? (
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    handleRestore(item.indirectWorkCaseId)
                  }}
                  title="復元"
                >
                ...
                <div className="flex gap-0.5 shrink-0">
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      setDeleteTargetId(item.indirectWorkCaseId)
                    }}
                    title="削除"
                  >
