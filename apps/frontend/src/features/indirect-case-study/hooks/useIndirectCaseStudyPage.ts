...
export function useIndirectCaseStudyPage({ businessUnitCode }: UseIndirectCaseStudyPageParams) {
  // 選択状態
  const [selectedHeadcountPlanCaseId, setSelectedHeadcountPlanCaseId] = useState<number | null>(
    null,
  )
  const [selectedCapacityScenarioId, setSelectedCapacityScenarioId] = useState<number | null>(null)
  const [selectedIndirectWorkCaseId, setSelectedIndirectWorkCaseId] = useState<number | null>(null)

  // 年度管理
  const [settingsFiscalYear, setSettingsFiscalYear] = useState(getCurrentFiscalYear())
  const [resultFiscalYear, setResultFiscalYear] = useState(getCurrentFiscalYear())

  // 計算結果
  const [capacityResult, setCapacityResult] = useState<CalculateCapacityResult | null>(null)
  const [indirectWorkResult, setIndirectWorkResult] = useState<IndirectWorkCalcResult | null>(null)
  ...
  const [ratioLocalData, setRatioLocalData] = useState<BulkIndirectWorkRatioInput | null>(null)
  ...
  const monthlyHeadcountQuery = useQuery(
    monthlyHeadcountPlansQueryOptions(selectedHeadcountPlanCaseId ?? 0, businessUnitCode),
  )
  ...
  }, [selectedCapacityScenarioId, selectedHeadcountPlanCaseId, businessUnitCode, capacityCalc])
  ...
  const saveIndirectWorkLoads = useCallback(async () => {
    ...
    setIndirectWorkResultDirty(false)
  }, [selectedIndirectWorkCaseId, indirectWorkResult, bulkIndirectWorkLoadsMutation])
