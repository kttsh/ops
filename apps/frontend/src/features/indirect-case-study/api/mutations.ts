...
export function useCreateHeadcountPlanCase() {
  ...
  return useMutation({
    mutationFn: (input: CreateHeadcountPlanCaseInput) => createHeadcountPlanCase(input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: indirectCaseStudyKeys.headcountPlanCases() })
...
export function useUpdateHeadcountPlanCase() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, input }: { id: number; input: UpdateHeadcountPlanCaseInput }) =>
...
export function useBulkUpdateMonthlyHeadcountPlans() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ caseId, input }: { caseId: number; input: BulkMonthlyHeadcountInput }) =>
      bulkUpdateMonthlyHeadcountPlans(caseId, input),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: [...indirectCaseStudyKeys.headcountPlanCases(), variables.caseId, 'monthly'],
      })
      toast.success('保存しました')
    },
    onError: () => {
...
export function useCreateCapacityScenario() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (input: CreateCapacityScenarioInput) => createCapacityScenario(input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: indirectCaseStudyKeys.capacityScenarios() })
      toast.success('保存しました')
    },
    onError: () => {
      toast.error('保存に失敗しました')
...
export function useUpdateCapacityScenario() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, input }: { id: number; input: UpdateCapacityScenarioInput }) =>
...
export function useCreateIndirectWorkCase() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (input: CreateIndirectWorkCaseInput) => createIndirectWorkCase(input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: indirectCaseStudyKeys.indirectWorkCases() })
      toast.success('保存しました')
    },
    onError: () => {
      toast.error('保存に失敗しました')
...
export function useUpdateIndirectWorkCase() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, input }: { id: number; input: UpdateIndirectWorkCaseInput }) =>
...
export function useBulkUpdateIndirectWorkTypeRatios() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ caseId, input }: { caseId: number; input: BulkIndirectWorkRatioInput }) =>
      bulkUpdateIndirectWorkTypeRatios(caseId, input),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: indirectCaseStudyKeys.indirectWorkTypeRatios(variables.caseId),
      })
      toast.success('保存しました')
    },
    onError: () => {
...
export function useBulkSaveMonthlyIndirectWorkLoads() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: [...indirectCaseStudyKeys.indirectWorkCases(), variables.caseId, 'monthly-loads'],
      })
      toast.success('保存しました')
    },
    onError: () => {
