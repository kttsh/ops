...
export async function fetchBusinessUnitsForSelect(): Promise<PaginatedResponse<BusinessUnit>> {
  const searchParams = new URLSearchParams({
    'page[number]': '1',
    'page[size]': '1000',
  })
...
export async function fetchProjectTypesForSelect(): Promise<PaginatedResponse<ProjectType>> {
