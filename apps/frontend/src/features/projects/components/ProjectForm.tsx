  ...
  return (
    ...
    >
      ...
      <form.Field
        name="projectCode"
        validators={{
          onChange: mode === 'create' ? createProjectSchema.shape.projectCode : undefined,
        }}
      >
        {(field) => (
          <div className="space-y-2">
            ...
            {field.state.meta.errors.length > 0 && (
              <p className="text-sm text-destructive">{getErrorMessage(field.state.meta.errors)}</p>
            )}
          </div>
        )}
      </form.Field>
          ...
          <div className="space-y-2">
            ...
            {buQuery.isLoading ? (
              ...
              <div className="flex items-center gap-2 h-10">
                ...
                <Button type="button" variant="ghost" size="sm" onClick={() => buQuery.refetch()}>
                  再試行
                </Button>
              </div>
            ) : (
        ...
        {(field) => (
          <div className="space-y-2">
            ...
            {ptQuery.isLoading ? (
              ...
              <div className="flex items-center gap-2 h-10">
                <p className="text-sm text-destructive">選択肢の取得に失敗しました</p>
                <Button type="button" variant="ghost" size="sm" onClick={() => ptQuery.refetch()}>
                  再試行
                </Button>
              </div>
            ) : (
      ...
      <form.Field
        ...
        validators={{
          onChange: ({ value }) => {
            if (typeof value !== 'number' || Number.isNaN(value)) return '総工数は数値で入力してください'
      ...
      <form.Field
        ...
        validators={{
          onChange: ({ value }) => {
            ...
            if (typeof value !== 'number' || Number.isNaN(value)) return '期間月数は数値で入力してください'
            if (!Number.isInteger(value)) return '期間月数は整数で入力してください'
