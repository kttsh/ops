...
- [x] 1.1 (P) キャパシティシナリオのZodスキーマとTypeScript型を定義する
  - 作成用スキーマ: シナリオ名（必須、1〜100文字）、プライマリフラグ（任意、デフォルトfalse）、説明（任意、最大500文字、null許可）
  - 更新用スキーマ: すべてのフィールドを任意とし、シナリオ名のみ指定時は1〜100文字を検証
  - 一覧取得クエリスキーマ: 既存のページネーションスキーマを拡張し、論理削除済みを含めるフィルタを追加
  - DB行型（snake_case）: capacity_scenario_id, scenario_name, is_primary, description, created_at, updated_at, deleted_at
  - APIレスポンス型（camelCase）: capacityScenarioId, scenarioName, isPrimary, description, createdAt, updatedAt
...
- [x] 2.1 一覧取得と単一取得のデータアクセスを実装する
  - ページネーション付き一覧取得: 単一テーブルからのSELECT、ソフトデリートフィルタを適用、OFFSET/FETCHでページング
...
- [x] 2.2 作成・更新のデータアクセスを実装する
  - 作成: シナリオ名、プライマリフラグ、説明をINSERTし、OUTPUT句でIDを取得後、findByIdで完全な行を返却
...
- [x] 2.3 論理削除・復元・参照チェックのデータアクセスを実装する
  ...
  - 参照チェック: monthly_capacityテーブルに対象IDを参照するレコードが存在するかEXISTSで確認
...
- [x] 3.2 作成・更新のビジネスロジックを実装する
  - 作成: 外部キーを持たないため、FK存在チェック不要。データ層のcreateを直接呼び出し、変換後に返却
  - 更新: データ層のupdateを呼び出し、結果がなければ404例外をスロー
...
- [x] 4.2 アプリケーションにルートをマウントする
  - エントリーポイントにcapacity-scenariosルートをインポートし、`/capacity-scenarios`パスでマウント
